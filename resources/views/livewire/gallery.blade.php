<div>
        ...
        <table class="w-full text-sm text-left">
            ...
            <tbody>
                ...
                <tr class="bg-white border-b-2  hover:bg-gray-200 py-2">
                    ...
                    <td class="py-2">
                        <button wire:click="add({{$item->id}})" type="button" data-bs-toggle="modal"
                            title="Agregar mas imagenes" data-bs-target="#addImages"><i
                                class="fa-regular fa-images"></i></button>
                        <button wire:click="edit({{$item->id}})" type="button" data-bs-toggle="modal"
                            title="Editar Titulo y descripcion" data-bs-target="#editImages"><i
                                class="fa-solid fa-pen m-1"></i></button>
                        <button wire:click="del({{$item->id}})" type="button" data-bs-toggle="modal"
                            title="Eliminar galeria" data-bs-target="#delImages"><i
                                class="fa-solid fa-trash-can m-1"></i></button>
                    </td>
                </tr>
                @endforeach
            </tbody>
    ...
    <div wire:ignore.self
        ...
        id="createGallery" tabindex="-1" aria-labelledby="createGalleryLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="mx-auto shadow-xl rounded-md bg-white max-w-xl">
                <div class="flex justify-between items-center bg-aside text-white text-xl rounded-t-md px-4 py-2">
                    <h1 class="modal-title fs-5" id="createGalleryLabel">Nueva Galeria</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"><i
                            class="fa-solid fa-x"></i></button>
                </div>
                <div class="p-4">
                    <form wire:submit.prevent="storeNewGallery" enctype="multipart/form-data">
                        @csrf
                        <div class="grid">
                            ...
                            <span class="text-red-600">{{$message}}</span>
                            @enderror
                            <label class="block tracking-wide font-bold mb-2">Imagenes</label>
                            <p class="text-sm text-gray-500 mb-1">PNG, JPG o JPEG Recomendado 1000 x 1000 o superior</p>
                            <input type="file" name="files" wire:model="files" multiple
                            ...
                            <div wire:loading wire:target="files">Cargando...</div>
                            @error('files')
                            <span class="text-red-600">{{$message}}</span>
                            @enderror
                        </div>
    ...
    <div wire:ignore.self
        class="fixed hidden z-40 inset-0 bg-gray-900 bg-opacity-60 overflow-y-auto h-full w-full px-4 modal"
        id="addImages" tabindex="-1" aria-labelledby="addImagesLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="mx-auto shadow-xl rounded-md bg-white max-w-xl">
                <div class="flex justify-between items-center bg-aside text-white text-xl rounded-t-md px-4 py-2">
                    <h1 class="modal-title fs-5" id="addImagesLabel">Agregar mas imagenes</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"><i
                            class="fa-solid fa-x"></i></button>
                </div>
                <div class="p-4">
                    <form wire:submit.prevent="addImagesData" enctype="multipart/form-data">
                        @csrf
                        <div class="grid">
                            <label for="fileAdd"
                                class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-500 border-dashed rounded-lg cursor-pointer">
                                <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                    <i class="fa-solid fa-upload fa-2xl"></i><br>
                                    <p class="mb-2 text-sm text-gray-600 "><span class="font-semibold">Agrega mas fotos
                                            a la galeria</p>
                                    <p class="text-xs text-gray-500 ">PNG, JPG o JPEG Recomendado 1000 x 1000 o superior
                                    </p>
                                </div>
                                <input id="fileAdd" name="files" type="file" wire:model="files" class="hidden" multiple>
                                <div wire:loading wire:target="files">Cargando...</div>
                            </label>
                            @error('files')
                            <span class="text-red-600">{{$message}}</span>
    ...
    <div wire:ignore.self
        class="fixed hidden z-40 inset-0 bg-gray-900 bg-opacity-60 overflow-y-auto h-full w-full px-4 modal"
        id="editImages" tabindex="-1" aria-labelledby="editImagesLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="mx-auto shadow-xl rounded-md bg-white max-w-xl">
                <div class="flex justify-between items-center bg-aside text-white text-xl rounded-t-md px-4 py-2">
                    <h1 class="modal-title fs-5" id="editImagesLabel">Editar Galeria</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"
                        wire:click="resert"><i class="fa-solid fa-x"></i></button>
                </div>
                <div class="p-4">
                    <form wire:submit.prevent="editImagesData" enctype="multipart/form-data">
                        @csrf
                        <div class="grid">
                            <label class="block tracking-wide font-bold mb-2">Titulo</label>
                            <input type="text" name="title" wire:model="title"
    ...
    <div wire:ignore.self
        class="fixed hidden z-40 inset-0 bg-gray-900 bg-opacity-60 overflow-y-auto h-full w-full px-4 modal"
        id="delImages" tabindex="-1" aria-labelledby="delImagesLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="mx-auto shadow-xl rounded-md bg-white max-w-xl">
                <div class="flex justify-between items-center bg-aside text-white text-xl rounded-t-md px-4 py-2">
                    <h1 class="modal-title fs-5" id="delImagesLabel">Eliminar Galeria</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"><i
                            class="fa-solid fa-x"></i></button>
                </div>
                <div class="p-4">
                    <h1 class="text-center text-xl p-6">🚨¿Estas seguro de eliminar este registro?🚨</h1>
                    <form wire:submit.prevent="delImagesData" enctype="multipart/form-data">
                        @csrf
